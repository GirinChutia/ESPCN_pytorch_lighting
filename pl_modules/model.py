...
class ESPCN_model(nn.Module):
    def __init__(self, scale : int, channels : int=3) -> None:
        super().__init__()
        self.conv_1 = nn.Conv2d(in_channels=channels, out_channels=64, kernel_size=5, padding=2)
        ...
        self.conv_3 = nn.Conv2d(in_channels=32, out_channels=(channels * scale * scale), kernel_size=3, padding=1)
